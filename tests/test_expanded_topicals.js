...
assert.strictEqual(context.metadata.question_count, 15009);

const expected = {
    math_ai: { topics: 78, uniqueQuestions: 294 },
    business: { topics: 37, uniqueQuestions: 231 },
    economics: { topics: 31, uniqueQuestions: 276 },
    computer_science: { topics: 8, uniqueQuestions: 583 }
};
...
for (const [subject, counts] of Object.entries(expected)) {
    ...
    assert(questions.some(question => question.filepath.startsWith(
        `Content/TopicQuestionBank/2026-09-19-new-papers-v1/${subject}/questions/`
    )));
    assert(questions.some(question => question.full_paper_path.startsWith(
...
for (const subject of Object.keys(expected)) {
    ...
    assert(appSource.includes(`mergeFullPaperData(additionalSubjectFullPapersData.${subject}, newPaperFullPapersData.${subject})`));
}
